...
$(document).ready(function ()
{
    ...
    $("#btnEarlyNumFilter").on("click", filterEarlyNum);
    ...
    $("#btnPrsistAssessmentCSV").on("click", prsistAssessmentCSV);
    ...
    $("#btnEarlyNumCSV").on("click", earlynumCSV);
    ...
    if (route.indexOf("/vocab") > -1 || route.indexOf("/cardsort") > -1 || route.indexOf("/mrant") > -1 || route.indexOf("/fishshark") > -1 || route.indexOf("/notthis") > -1 || route.indexOf("/ecers") > -1 || route.indexOf("/questionnaire") > -1 || route.indexOf("/early_numeracy") > -1) {
        formSetup();
    }
...
function updateGame()
{
    ...
    $.ajax({
        ...
        data: {
            ...
            game_id     : game_id,
            type        : game_type,
            centre      : $("[name=centre]").val(),
            room        : $("[name=room]").val(),
            assessor    : $("[name=assessor]").val(),
            observer    : $("[name=observer]").val(),
...
function earlynumCSV()
{
    var test = $("#test_name").val();
    var start = $("#date_start");
    var end = $("#date_end");
    var game = "/early_numeracy";
...
function uploadGameData()
{
    ...
    $("#frmGameData").ajaxForm({
        ...
        complete: function(data) {
			...
			var formData = {};
			if (game_type == "questionnaire") {
            	formData.results = [gameData];
            }
...
function filterEarlyNum()
{
    $.colorbox({
        onOpen: function ()
        {
            window.location.pathname = filterURL("early_numeracy");
        }
    });
}
...
function filterPrsistAssessment()
{
    $.colorbox({
        onOpen: function ()
        {
            window.location.pathname = filterURL("prsistassessment");
        }
    });
}
...
function questionnaireCSV()
{
    ...
    var start = $("#date_start");
    var end = $("#date_end");
	var url = (start.val() == "" && end.val() == "") ? "/questionnaire/csv/" + test : "/questionnaire/csv/" + test + "/" + getDate(start) + "/" + getDate(end) + "/";
...
function prsistAssessmentCSV()
{
    var test = $("#test_name").val();
    var start = $("#date_start");
    var end = $("#date_end");
    var url = (start.val() == "" && end.val() == "") ? "/prsistassessment/csv/" + test : "/prsistassessment/csv/" + test + "/" + getDate(start) + "/" + getDate(end) + "/";
        ...
        $.ajax({
            ...
            complete: function (data)
            {
                ...
                if (json.success) {
                    ...
                    renderAlert("danger", "Access Denied");
                }
            }
        });
    }
