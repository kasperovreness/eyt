...
class QuestionnaireController extends Controller
{
    ...
    private function getResults($test_name = null, $start = null, $end = null)
    {
        ...
        if (!empty($test_name) && ($test_name != "all") && !empty($start) && !empty($end)) {
        ...
        } else if (!empty($test_name) && ($test_name == "all") && !empty($start) && !empty($end)) {
        	$results = Questionnaire::where("played_at", ">=", $start)->where("played_at", "<=", $end)->orderBy($order, "DESC")->get();
        } else {
    ...
    public function fixDuplicates()
    {
        ...
        foreach ($games as $game) {
            ...
            $duplicates = Questionnaire::where("id", "!=", $game->id)->where("subject_id", "=", $game->subject_id)->where("session_id", "=", $game->session_id)->where("test_name", "=", $game->test_name)->where("played_at", "=", $game->played_at)->get();

            foreach ($duplicates as $duplicate) {
            	$num_deleted = $num_deleted + 1;
            	$deleted[] = $duplicate->id;
            	
                QuestionnaireAnswer::where("game_id", "=", $duplicate->id)->delete();
                Questionnaire::where("id", "=", $duplicate->id)->delete();
            }
        }
